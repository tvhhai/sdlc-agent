...
describe("real agents — claude-code adapter", () => {
	...
	let supportFiles: OutputFile[];
	// Agent + command files are the "agent-shaped" outputs that carry frontmatter,
	// an [SDLC:] prefix, and the generated-by footer. Support files are raw source
	// docs (templates/policies) and must be excluded from those assertions.
	let bundleFiles: OutputFile[];
	...
	beforeAll(() => {
		...
		supportFiles = outputs.filter((o) => o.path.startsWith(".claude/sdlc/"));
		bundleFiles = [...agentFiles, ...commandFiles];
	});

	it("produces 6 agents + 6 commands + a non-empty support bundle", () => {
		...
		expect(outputs).toHaveLength(
			agentFiles.length + commandFiles.length + supportFiles.length,
		);
	});

	it("bundle is self-contained: every referenced support file is emitted, no root refs", () => {
		const emitted = new Set(supportFiles.map((o) => o.path));
		for (const f of agentFiles) {
			// References inside agent bodies must point at the bundled copies…
			const refs = [...f.content.matchAll(/`(\.claude\/sdlc\/[^`]+)`/g)].map(
				(m) => m[1],
			);
			for (const ref of refs) {
				expect(emitted.has(ref), `${f.path} references un-emitted ${ref}`).toBe(
					true,
				);
			}
			// …and never at the root templates/ or policies/ dirs.
			expect(f.content, `${f.path} has a root policies/ ref`).not.toMatch(
				/`policies\//,
			);
			expect(f.content, `${f.path} has a root templates/ ref`).not.toMatch(
				/`templates\//,
			);
		}
	});

	it("support files live under .claude/sdlc/{templates,policies} and match committed source", () => {
		for (const f of supportFiles) {
			expect(f.path).toMatch(
				/^\.claude\/sdlc\/(templates|policies)\/[a-z][a-z0-9-]+\.md$/,
			);
		}
	});
	...
	it("frontmatter is valid YAML for every agent + command file", () => {
	...
	it("frontmatter description has [SDLC:<phase>] prefix on every agent + command file", () => {
		for (const f of bundleFiles) {
	...
	it("every agent + command file ends with the generated-by footer", () => {
		for (const f of bundleFiles) {
